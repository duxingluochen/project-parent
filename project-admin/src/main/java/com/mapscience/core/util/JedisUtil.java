...
import com.mapscience.core.common.Constant;
...
public class JedisUtil {
    ...
    public static <T> T get(String key , Class<T>...requiredType){
       ...
       // boolean isBroken = false;
        try {
            ...
            jds.select(0);
            byte[] skey = com.mapscience.core.util.SerializableUtil.serialize(key);
            return SerializableUtil.deserialize(jds.get(skey),requiredType);
        } catch (Exception e) {
            //isBroken = true;
            e.printStackTrace();
        } finally {
    ...
    public static void set(Object key ,Object value){
        ...
        //boolean isBroken = false;
        try {
            jds = jds = jedisPool.getResource();
            jds.select(0);
            byte[] skey = SerializableUtil.serialize(key);
            byte[] svalue = SerializableUtil.serialize(value);
            jds.set(skey, svalue);
        } catch (Exception e) {
            //isBroken = true;
            e.printStackTrace();
